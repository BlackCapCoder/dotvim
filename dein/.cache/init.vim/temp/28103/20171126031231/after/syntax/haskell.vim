...
if !Cf('𝐒')
    ...
    syntax match hsNiceOperator "\<String\>"  conceal cchar=S
endif

" '𝐓' option to disable Text type to 𝐓 concealing
if !Cf('𝐓')
    " syntax match hsNiceOperator "\<Text\>"    conceal cchar=𝐓
    syntax match hsNiceOperator "\<Text\>"    conceal cchar=T
endif
...
if !Cf('𝐄')
    ...
    syntax match hsNiceOperator "\<Either\>"  conceal cchar=E
    syntax match hsNiceOperator "\<Right\>"   conceal cchar=R
    syntax match hsNiceOperator "\<Left\>"    conceal cchar=L
endif

" '𝐌' option to disable Maybe/Just/Nothing to 𝐌/𝐽/𝑁 concealing
if !Cf('𝐌')
    syntax match hsNiceOperator "\<Maybe\>"   conceal cchar=M
    ...
    syntax match hsNiceOperator "\<Just\>"    conceal cchar=J
    syntax match hsNiceOperator "\<Nothing\>" conceal cchar=N
endif
...
if Cf('s')
    ...
    syntax match hsNiceOperator "\<succ\>[.\s\S\n]"              conceal cchar=⧽
    syntax match hsNiceOperator "\<pred\>[.\s\S\n]"              conceal cchar=⧼
    " syntax match hsNiceOperator "\<succ\>"                       conceal cchar=⧽
    " syntax match hsNiceOperator "\<pred\>"                       conceal cchar=⧼

    " syntax match variable "\<f\>"                               conceal cchar=ƒ
    " syntax match variable "\<i\>"                               conceal cchar=ι
endif
...
elseif !Cf('t')
    syntax match hsNiceSpecial "\<True\>"  conceal cchar=T
    syntax match hsNiceSpecial "\<False\>" conceal cchar=F
endif
